...
async function main() {
  ...
  const zonesData = [
    ...
    },
    {
      ...
      subTabs: [
        { id: 'all', label: 'All', bias: null },
        { id: 'seasonal', label: 'Seasonal', bias: null },
        { id: 'limited', label: 'Limited', bias: null },
      ],
      sortOrder: 5,
      isActive: true,
    },
  ...
  for (const zoneData of zonesData) {
    ...
    if (!existing) {
      ...
      console.log(`  ⊘ Skipped (exists): ${zoneData.name}`);
    }
  }

  console.log(`\n✅ Seeded ${zoneCount} zones (${zonesData.length - zoneCount} already existed)\n`);
