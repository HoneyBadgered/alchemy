...
import { ZONE_CONFIGS, CAFFEINE_ICONS } from '../zone-config';
...
const ZONE = ZONE_CONFIGS.observatory;
...
export default function ObservatoryPage() {
  ...
  const { data, isLoading } = useQuery<ProductsResponse>({
    ...
    queryFn: async () => {
      ...
      params.set('zone', 'The Observatory');
    ...
    <div className={`min-h-screen bg-gradient-to-b ${ZONE.bgGradient}`}>
      ...
      <div className={`relative mt-16 overflow-hidden bg-gradient-to-r ${ZONE.gradient}`}>
        <div className="absolute inset-0 opacity-20">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_120%,rgba(150,100,255,0.3),transparent_50%)]" />
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_80%_20%,rgba(180,120,255,0.2),transparent_40%)]" />
        </div>
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-20 text-center">
          ...
          <p className="text-2xl text-violet-100 italic font-light mb-2">
            {ZONE.tagline}
          </p>
      ...
      <div className="bg-black/40 backdrop-blur-sm border-b border-white/10 sticky top-16 z-40">
        <div className="max-w-7xl mx-auto px-6 py-3">
          <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
            ...
            <Link href="/shop/conservatory" className="px-4 py-2 rounded-md text-sm font-medium whitespace-nowrap bg-white/10 text-white/70 hover:bg-white/20 transition">The Conservatory</Link>
            ...
            <Link href="/shop/observatory" className="px-4 py-2 rounded-md text-sm font-medium whitespace-nowrap bg-violet-700 text-white">The Observatory</Link>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                    ...
                    {availableFilters.teaTypes.map(type => (
                      <label key={type} className="flex items-center cursor-pointer group">
                        <input
                          ...
                          className="mr-3 rounded border-white/30 bg-white/10 text-violet-600 focus:ring-violet-500"
                        />
          ...
          <div className="lg:col-span-3">
            ...
            >
              <span>Filters</span>
              {activeFilterCount > 0 && (
                <span className="bg-violet-700 text-white text-xs px-2 py-1 rounded-full">
                  {activeFilterCount}
                </span>
              )}
            </button>
            ...
            {isLoading ? (
              ...
              <div className="text-center py-20">
                ...
                <button onClick={resetFilters} className="text-violet-400 hover:text-violet-300 underline">
                  Clear filters
                </button>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredProducts.map(product => (
                  <Link
                    ...
                    className="group bg-white/5 backdrop-blur-sm rounded-xl overflow-hidden border border-white/10 hover:border-violet-500/50 transition-all hover:shadow-xl hover:shadow-violet-900/20"
                  >
                    <div className="aspect-square bg-gradient-to-br from-indigo-950/40 to-violet-950/40 relative overflow-hidden">
                    ...
                    <div className="p-5">
                      <h3 className="text-lg font-serif font-semibold text-white mb-2 group-hover:text-violet-400 transition">
                        {product.name}
                      </h3>
                      ...
                      {product.flavorNotes && product.flavorNotes.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 mb-3">
                          {product.flavorNotes.slice(0, 3).map(note => (
                            <span
                              key={note}
                              className="text-xs px-2 py-1 rounded-full bg-violet-900/30 text-violet-200 capitalize"
                            >
                              {note}
                            </span>
                          ))}
                      ...
                      <div className="flex items-center justify-between pt-3 border-t border-white/10">
                        ...
                        <span className="text-sm text-violet-400 group-hover:text-violet-300 transition font-medium">
                          View Blend →
                        </span>
                      </div>
                    </div>
        ...
        <div className="mt-16 text-center py-12 border-t border-white/10">
          ...
          <Link
            href="/table"
            className="inline-block px-8 py-3 bg-gradient-to-r from-violet-700 to-purple-700 text-white font-semibold rounded-full hover:shadow-lg hover:shadow-violet-900/30 transition-all hover:scale-105"
          >
