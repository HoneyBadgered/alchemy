...
const ZONE = ZONE_CONFIGS['east-pavilion'];

export default function EastPavilionPage() {
    ...
    <div className={`min-h-screen bg-gradient-to-b ${ZONE.bgGradient}`}>
      ...
      <div className={`relative mt-16 overflow-hidden bg-gradient-to-r ${ZONE.gradient}`}>
        <div className="absolute inset-0 opacity-20">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_120%,rgba(150,220,255,0.3),transparent_50%)]" />
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_80%_20%,rgba(100,180,255,0.2),transparent_40%)]" />
        </div>
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-20 text-center">
          ...
          <p className="text-2xl text-cyan-100 italic font-light mb-2">
            {ZONE.tagline}
          </p>
      ...
      <div className="bg-black/40 backdrop-blur-sm border-b border-white/10 sticky top-16 z-40">
        <div className="max-w-7xl mx-auto px-6 py-3">
          <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
            ...
            <Link href="/shop/east-pavilion" className="px-4 py-2 rounded-md text-sm font-medium whitespace-nowrap bg-cyan-600 text-white">The East Pavilion</Link>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
                    ...
                    {availableFilters.teaTypes.map(type => (
                      <label key={type} className="flex items-center cursor-pointer group">
                        <input
                          ...
                          className="mr-3 rounded border-white/30 bg-white/10 text-cyan-600 focus:ring-cyan-500"
                        />
          ...
          <div className="lg:col-span-3">
            ...
            {isLoading ? (
              ...
              <div className="text-center py-20">
                ...
                <button onClick={resetFilters} className="text-cyan-400 hover:text-cyan-300 underline">
                  Clear filters
                </button>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredProducts.map(product => (
                  <Link
                    ...
                    className="group bg-white/5 backdrop-blur-sm rounded-xl overflow-hidden border border-white/10 hover:border-cyan-500/50 transition-all hover:shadow-xl hover:shadow-cyan-900/20"
                  >
                    <div className="aspect-square bg-gradient-to-br from-cyan-950/40 to-blue-950/40 relative overflow-hidden">
                    ...
                    <div className="p-5">
                      <h3 className="text-lg font-serif font-semibold text-white mb-2 group-hover:text-cyan-400 transition">
                        {product.name}
                      </h3>
                      ...
                      {product.flavorNotes && product.flavorNotes.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 mb-3">
                          {product.flavorNotes.slice(0, 3).map(note => (
                            <span
                              key={note}
                              className="text-xs px-2 py-1 rounded-full bg-cyan-900/30 text-cyan-200 capitalize"
                            >
                              {note}
                            </span>
                          ))}
                      ...
                      <div className="flex items-center justify-between pt-3 border-t border-white/10">
                        ...
                        <span className="text-sm text-cyan-400 group-hover:text-cyan-300 transition font-medium">
                          View Blend →
                        </span>
                      </div>
                    </div>
        ...
        <div className="mt-16 text-center py-12 border-t border-white/10">
          ...
          <Link
            href="/table"
            className="inline-block px-8 py-3 bg-gradient-to-r from-cyan-600 to-blue-600 text-white font-semibold rounded-full hover:shadow-lg hover:shadow-cyan-900/30 transition-all hover:scale-105"
          >
