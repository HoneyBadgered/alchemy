...
function OrderHistoryContent() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'shipped':
        return 'bg-[var(--primary)]/50 text-[var(--accent)] border-[var(--primary)]/50';
      case 'cancelled':
      case 'payment_failed':
    ...
    <div className="min-h-screen bg-[var(--background)] pb-20">
      ...
      <div className="relative bg-[var(--surface)]/80 border-b border-[var(--primary)]/30">
        <div className="max-w-5xl mx-auto px-4 py-6">
          ...
          <p className="text-[var(--text-muted)] mt-1">Chronicles of your alchemical acquisitions</p>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-6 relative z-10">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-all duration-300 ${
                  statusFilter === option.value
                    ...
                    : 'bg-[var(--surface)]/60 text-[var(--accent)] hover:bg-slate-700/60 border border-[var(--primary)]/20'
                }`}
              >
                {option.label}
              </button>
        ...
        {isLoading && (
          <div className="flex justify-center items-center py-20">
            <div className="text-[var(--accent)] text-lg">Summoning your order history...</div>
          </div>
        )}
        ...
        {data && data.orders.length === 0 && (
          <div className="bg-[var(--surface)]/60 backdrop-blur-sm rounded-xl p-8 text-center border border-[var(--primary)]/20">
            ...
            <Link
              href="/shop"
              className="inline-block bg-[var(--primary)] hover:bg-[var(--primary-hover)] text-white px-6 py-3 rounded-lg font-semibold transition-colors"
            >
        ...
        {data && data.orders.length > 0 && (
          <div className="space-y-4">
            {data.orders.map((order) => (
              <div
                key={order.id}
                className="bg-[var(--surface)]/60 backdrop-blur-sm rounded-xl overflow-hidden border border-[var(--primary)]/20 hover:border-[var(--primary)]/40 transition-all duration-300"
              >
                {/* Order Header */}
                <div className="p-5 border-b border-[var(--primary)]/20">
                ...
                <div className="p-5 bg-slate-900/30">
                  <div className="flex gap-3 overflow-x-auto pb-2">
                    {order.order_items.slice(0, 4).map((item) => (
                      <div
                        key={item.id}
                        className="flex-shrink-0 w-16 h-16 bg-slate-700/50 rounded-lg overflow-hidden border border-[var(--primary)]/20"
                    ...
                    {order.order_items.length > 4 && (
                      <div className="flex-shrink-0 w-16 h-16 bg-slate-700/50 rounded-lg flex items-center justify-center border border-[var(--primary)]/20">
                ...
                {/* Order Actions */}
                <div className="p-5 flex flex-wrap gap-3 border-t border-[var(--primary)]/10">
                  <button
                    onClick={() => router.push(`/orders/${order.id}`)}
                    className="px-4 py-2 bg-[var(--primary)] hover:bg-[var(--primary-hover)] text-white rounded-lg font-medium transition-colors"
                  >
                    View Details
                  </button>
                  <button
                    ...
                    className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-[var(--text-base)] rounded-lg font-medium transition-colors disabled:opacity-50"
                  >
        ...
        {data && data.pagination.totalPages > 1 && (
          <div className="flex justify-center items-center gap-4 mt-8">
            <button
              ...
              disabled={page === 1}
              className="px-4 py-2 bg-[var(--surface)]/60 text-[var(--accent)] rounded-lg font-medium disabled:opacity-50 disabled:cursor-not-allowed hover:bg-slate-700/60 border border-[var(--primary)]/20 transition-colors"
            >
              ← Previous
            </button>
      ...
      {reorderModalOrder && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gradient-to-br from-slate-900 to-slate-800 border border-[var(--primary)]/30 rounded-2xl shadow-2xl max-w-md w-full p-6 transform transition-all">
            <div className="text-center mb-6">
              <div className="text-5xl mb-4">🔄</div>
              <h2 className="text-2xl font-bold text-[var(--text-base)] mb-2">
                Reorder Items?
              </h2>
            ...
            <div className="bg-slate-900/50 rounded-lg p-4 mb-6 max-h-48 overflow-y-auto">
              {reorderModalOrder.order_items.map((item, index) => (
                <div key={index} className="flex justify-between items-center py-2 border-b border-[var(--primary)]/20 last:border-0">
                  <div className="flex-1">
                    <span className="text-[var(--text-base)] text-sm block">{item.products?.name || 'Unknown Product'}</span>
                    <span className="text-[var(--text-muted)] text-xs">${Number(item.price).toFixed(2)} each</span>
                  </div>
                  <div className="text-right">
                    <span className="text-[var(--accent)] text-sm font-semibold block">×{item.quantity}</span>
                    <span className="text-[var(--text-base)] text-xs">${(Number(item.price) * item.quantity).toFixed(2)}</span>
                  </div>
                </div>
              ))}
              <div className="flex justify-between items-center pt-3 mt-3 border-t border-[var(--primary)]/30">
                <span className="text-[var(--text-base)] font-semibold">Total</span>
                <span className="text-[var(--text-base)] font-bold text-lg">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 px-4 py-3 bg-[var(--surface)]/60 hover:bg-slate-700/60 text-[var(--accent)] rounded-lg font-semibold transition-colors border border-[var(--primary)]/20"
              >
                Cancel
              </button>
              <button
                onClick={confirmReorder}
                className="flex-1 px-4 py-3 bg-[var(--primary)] hover:bg-[var(--primary-hover)] text-white rounded-lg font-semibold transition-all shadow-lg shadow-[var(--primary)]/30"
              >
                Add to Cart
              </button>
            </div>
